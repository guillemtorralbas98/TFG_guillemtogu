...
for i, nom_fitxer in enumerate(fitxers_parquet):
    ...
    if n_unics <= 2:
        ...
        if n_unics <= mida_mostra_dim:
            if matriu_per_dim is None:
                matriu_dim = calcular_matriu_distancies(coords_all, lens_all)
            else:
                matriu_dim = matriu_per_dim
        else:
            idx_mostra = np.random.choice(n_unics, size=mida_mostra_dim, replace=False)
            matriu_dim = calcular_matriu_distancies(
                coords_all[idx_mostra], lens_all[idx_mostra]
            )
