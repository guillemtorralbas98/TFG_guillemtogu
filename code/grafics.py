...
num_partides_ressaltar = min(5, len(partides_unicas))
ids_ressaltar = np.random.choice(partides_unicas, size=num_partides_ressaltar, replace=False)

fig = go.Figure()

for result, color in _COLOR_RESULT.items():
    mask_m = (resultats_mostra == result)
    if not mask_m.any():
        continue

    # Marcadors de fons (mostra)
    fig.add_trace(go.Scattergl(
        x=coords_mostra[mask_m, 0],
        y=coords_mostra[mask_m, 1],
        mode="markers",
        name=f"Resultat {result}",
        ...
        hovertemplate=(
            ...
            f"Resultat {result}<extra></extra>"
        ),
        customdata=np.column_stack([ids_mostra[mask_m], jugades_mostra[mask_m]]),
        legendgroup=result,
    ))

    mask_l = mask_partides_fons & (resultats_all == result)
    x_lin, y_lin = [], []
    for id_p in np.unique(ids_all[mask_l]):
        mask_p = (ids_all == id_p)
        ordre  = np.argsort(jugades_all[mask_p])
        x_lin.extend(coords_pts[mask_p, 0][ordre].tolist() + [np.nan])
        y_lin.extend(coords_pts[mask_p, 1][ordre].tolist() + [np.nan])

    fig.add_trace(go.Scattergl(
        x=x_lin, y=y_lin, mode="lines",
        name=f"Trajectòries {result}",
        opacity=0.08, line=dict(width=0.5, color=color),
        hoverinfo="skip", legendgroup=result, showlegend=False,
    ))
...
opacitats_normals = []
opacitats_atenuades = []
for result in _COLOR_RESULT.keys():
    mask_m = (resultats_mostra == result)
    if not mask_m.any():
        continue
    opacitats_normals.extend([0.6, 0.08])      
    opacitats_atenuades.extend([0.30, 0.05])   
...
prob_content_contour_level = 0.25
...
mask_mig_joc = fase_mostra == "Mig joc"
mask_dames_mig = {cat: mask_mig_joc & (categoria_dames_mostra == cat) for cat in colors_dames.keys()}
panell_kde(axos[2], mask_dames_mig, colors_dames, etiquetes_dames,
           "Per presència de dames al mig joc")
...
fig_kde.suptitle(
    f"Corbes de nivell al {int(prob_content_contour_level*100)}% de densitat",
    fontsize=14,
    fontweight="bold"
)
plt.tight_layout()
...
plt.close()
print(f"[OK] Bloc 3: gràfic KDE (3 panells) desat a: {fitxer_kde_sortida}")
...
fig_analitica.add_trace(go.Scatter(
    ...
    mode="lines+markers",
    name="Espai de ℝ pel 80% de variança",
